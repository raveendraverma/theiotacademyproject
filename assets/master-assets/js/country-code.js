...
// const input_box = document.querySelector('#tel');
...
//   const option = `
// <li class="option" id="option0">
//     <div>
//         <span class="iconify" id="iconify0" data-icon="flag:${country.code.toLowerCase()}-4x3"></span>
//         <span class="country-name" id="country-nmcd0">${country.name}</span>
//     </div>
//     <strong>+${country.phone}</strong>
// </li> `;

//   select_box.querySelector('ol').insertAdjacentHTML('beforeend', option);
//   options = document.querySelectorAll('#option0');
// }

// function selectOption() {
//   console.log(this);
//   const icon = this.querySelector('#iconify0').cloneNode(true),
...
//     let is_matched = option1
//       .querySelector('#country-nmcd')
//       .innerText.toLowerCase()
//       .includes(search_query1);
...
//     let is_matched = option2
//       .querySelector('#country-nmcd2')
//       .innerText.toLowerCase()
//       .includes(search_query2);
...
$(document).ready(function () {
  initializeCountrySelector('#selector1', countries);
  initializeCountrySelector('#selector2', countries);
  initializeCountrySelector('#selector3', countries);
  initializeCountrySelector('#selector4', countries);
  initializeCountrySelector('#selector5', countries);
});

function initializeCountrySelector(selectorId, countries) {
  const $selector = $(selectorId);

  populateCountryList($selector, countries);
  $selector.on('click', '.newSelectedOptions', function () {
    $selector.find('.flagList').toggle();
  });

  $selector.on('click', '.flagListItem', function () {
    const countryData = $(this).data('country');
    updateSelectedCountry($selector, countryData);
    $selector.find('.flagList').hide();
  });
}

function populateCountryList($selector, countries) {
  let element = '';
  for (let i = 0; i < countries.length; i++) {
    element += `
                <li class="flagListItem" data-country='${JSON.stringify(
                  countries[i]
                )}'>
                    <div>
                        <span class="iconify" data-icon="flag:${countries[
                          i
                        ].code.toLowerCase()}-4x3"></span>
                        <span class="country-name">${countries[i].name}</span>
                    </div>
                    <strong>+${countries[i].phone}</strong>
                </li>
                `;
  }
  $selector.find('.flagList').html(element);
}

function updateSelectedCountry($selector, country) {
  let newElement = `
            <div class="newSelectedOptions">
                <div class="iconAndflagPrnt">
                    <span class="iconify" data-icon="flag:${country.code.toLowerCase()}-4x3"></span>
                    <span class="country-name">+${country.phone}</span>
                </div>
            </div>
            `;
  $selector.find('.newSelectedOptions').replaceWith(newElement);
}
