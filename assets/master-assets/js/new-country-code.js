...
function initializeCountrySelector(selectorId, countries) {
  ...
  selector.addEventListener('click', function (event) {
    if (event.target.closest('.newSelectedOptions')) {
      selector.querySelector('.flagList').classList.toggle('show');
    } else if (event.target.closest('.flagListItem')) {
      const countryData = JSON.parse(
        event.target.closest('.flagListItem').getAttribute('data-country')
      );
      updateSelectedCountry(selector, countryData);
      selector.querySelector('.flagList').classList.remove('show');
    }
  });
}
...
function populateCountryList(selector, countries) {
  ...
  selector.querySelector('.flagList').innerHTML = element;
}
...
function updateSelectedCountry(selector, country) {
  let newElement = `
            ...
            `;
  selector.querySelector('.newSelectedOptions').outerHTML = newElement;
}
