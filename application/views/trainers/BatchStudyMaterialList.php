...
<div style="display: inline-block; float: right;margin-bottom: 10px;">
    <button type='button' class='btn-danger' onclick="delRecord();">Unassign</button>
        <a href="<?php echo base_url(); ?>StudyMaterialcnt">
...
function getbatchAssignList(){
    ...
    $.ajax({
        url: "<?php echo base_url();?>StudyMaterialcnt/batchAssignList",
        type: "post",
        async: false,
        success: function(feedback){
            ...
            try{
                ...
                for(var i in arr){  
                  appendme="<tr studymaterial_id='" + arr[i]['studymaterial_id'] + "' batch_id='" + arr[i]['batch_id'] + "'>";
                  appendme+="<td>"+arr[i]['batch_name']+"</td>";
                  appendme+="<td>"+arr[i]['studymaterial_title']+"</td>";
...
function delRecord(){
   ...
   $("#assignment-rows").find(".check").each(function(){
    ...
    if(confirm("Do you want to unassign?")){
        var par,studymaterial_id,batch_id;
        ...
        $("#assignment-rows").find(".check").each(function(){
            if($(this).prop("checked")){
                  ...
                  $.ajax({
                        url: "<?php echo base_url();?>StudyMaterialcnt/batchAssignDelete?studymaterial_id="+studymaterial_id+"&batch_id="+batch_id,
                        type: "post",
                        async: false,
                        success: function(feedback){
