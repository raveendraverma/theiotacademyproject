...
<div class="form-control" style="margin-bottom: 5px; width: 800px; display: inline-block; border: none; box-shadow: none;">
    ...
    <select id="topic-list" style="width:150px; display: inline-block;" name="topic_id" onchange="getassignmentList(this.value);"></select>
</div>
...
function getassignmentList(topic_id){
    var appendme;
    $("#errMsg").html("");
    $("#assignment-rows").html("");
    $.ajax({
        url: "<?php echo base_url();?>Assignmentcnt/subassignmentList?topic_id="+topic_id,
        type: "post",
        async: false,
        success: function(feedback){
            ...
            try{
                var arr = JSON.parse(feedback);
                for(var i in arr){  
                  appendme="<tr assignment_id='"+arr[i]['assignment_id']+"'>";
                  appendme+="<td>"+i+"</td>";
                  appendme+="<td class='assignment_title'>"+arr[i]['assignment_title']+"</td>";
                  ...
                  appendme+="&nbsp;<a href='<?php echo base_url()?>Assignmentcnt/assignmentForm?assignment_id="+arr[i]['assignment_id']+"'><button type='button' class='btn-primary'>Edit</button></a>";
                  appendme+="&nbsp;<a href='<?php echo base_url()?>Pdfcnt?assignment_id="+arr[i]['assignment_id']+"'><button type='button' class='btn-primary'>View</button></a>";
...
function val_button(el){
    ...
    $(el).attr("href","<?php echo base_url();?>Assignmentcnt/assignmentForm?assignment_id=0&subject_id="+$("#subject-list").val()+"&subject_name=" + $('#subject-list option:selected').text()+"&module_id="+$("#module-list").val()+"&module_name=" + $('#module-list option:selected').text()+"&topic_id="+$("#topic-list").val()+"&topic_name=" + $('#topic-list option:selected').text());
    return true;
    ...
    if(confirm("Do you want to delete?")){
        ...
        $("#assignment-rows").find(".check").each(function(){
            if($(this).prop("checked")){
                ...
                $.ajax({
                    url: "Assignmentcnt/assignmentDelete?assignment_id="+assignment_id,
                    type: "post",
                    async: false,
                    success: function(feedback){
...
function modal_set(el){
    $("#myModal #student-modal").attr("href","<?php echo base_url()?>Studentcnt?assignment_id="+$(el).closest("tr").attr("assignment_id"));
    $("#myModal #batch-modal").attr("href","<?php echo base_url()?>Batchcnt?assignment_id="+$(el).closest("tr").attr("assignment_id"));
}
