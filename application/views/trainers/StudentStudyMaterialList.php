...
function getstudentAssignList(){
    ...
    $.ajax({
        url: "<?php echo base_url();?>StudyMaterialcnt/studentAssignList",
        type: "post",
        async: false,
        success: function(feedback){
            try{
                ...
                for(var i in arr){  
                  appendme="<tr studymaterial_id='" + arr[i]['studymaterial_id'] + "' student_id='" + arr[i]['student_id'] + "'>";
...
function delRecord(){
   ...
   $("#assignment-rows").find(".check").each(function(){
    ...
    if(confirm("Do you want to unassign?")){
        var par,studymaterial_id,student_id;
        ...
        $("#assignment-rows").find(".check").each(function(){
            if($(this).prop("checked")){
                  ...
                  $.ajax({
                        url: "<?php echo base_url();?>StudyMaterialcnt/studentAssignDelete?studymaterial_id="+studymaterial_id+"&student_id="+student_id,
                        type: "post",
                        async: false,
                        success: function(feedback){
