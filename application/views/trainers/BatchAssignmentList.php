...
function getbatchAssignList(){
    ...
    $.ajax({
        url: "<?php echo base_url();?>Assignmentcnt/batchAssignList",
        type: "post",
        async: false,
        success: function(feedback){
            ...
            try{
                ...
                for(var i in arr){  
                  appendme="<tr assignment_id='" + arr[i]['assignment_id'] + "' batch_id='" + arr[i]['batch_id'] + "'>";
...
function delRecord(){
   ...
   $("#assignment-rows").find(".check").each(function(){
    ...
    if(confirm("Do you want to unassign?")){
        var par,assignment_id,batch_id;
        ...
        $("#assignment-rows").find(".check").each(function(){
            if($(this).prop("checked")){
                  ...
                  $.ajax({
                        url: "<?php echo base_url();?>Assignmentcnt/batchAssignDelete?assignment_id="+assignment_id+"&batch_id="+batch_id,
                        type: "post",
                        async: false,
                        success: function(feedback){
