...
class Member extends CI_Model{
    ...
    {
        $query = $this->db->query("SELECT  * FROM members Where status = 1 ORDER BY id DESC");
        if ($query->num_rows()>0) 
        {
    ...
    {
        $query = $this->db->query("SELECT  * FROM members Where status = 0");
        if ($query->num_rows()>0) 
        {
            return $query->result_array();
        }
    ...
    public function fetch_search_data($query)
    {
        ...
        if($query != '')
        {
            ...
            $this->db->or_like('cert_image', $query);
     ...
     {
        $query = $this->db->query("SELECT * FROM `members` WHERE `id` IN ($Ids) LIMIT 30");
        if ($query->num_rows()>0) 
        {
            return $query->result();
        }
     ...
     {
        $query = $this->db->query("UPDATE `members` SET `mail_status`=1 WHERE `id` = '$ID'");
         if ($query) 
        {
            return true;
        }
