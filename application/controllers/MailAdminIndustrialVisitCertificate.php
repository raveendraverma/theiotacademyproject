...
class MailAdminIndustrialVisitCertificate extends CI_Controller{
...

    {
        ...
        $data = $this->IndustrialVisitAdminCertificateModel->fetch_member_email(implode(',', $checkid));
        $sent_id = [];
        $failed_id = [];
...

        {
            ...
            $subject = "Download Your Industrial Visit Certificate | The IoT Academy";
             $lnk1 = base_url('IndustrialVisitAdminCertificate/downloadpdfdetails/'.$value->id);
            ...
            Congratulations on participating in our Industrial Visit with The IoT Academy. Your participation certificate is now available for download. Click the download button below to access it.<br><br>
                  <a style='background:blue;color:white;padding:15px 30px;' href='".$lnk1."'><b>Download</b></a><br><br>
                     
             </p><p style='font-size:17px;'>
            Thank you for attending our Industrial Visit. We hope for your participation in future events too.
...

            {
                ...
                $this->IndustrialVisitAdminCertificateModel->user_mail_status($value->id);
