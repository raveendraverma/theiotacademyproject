...
class AdminController extends CI_Controller {
	...
	{ 
		...
		$this->load->view('admin/members/upload_form');
...

	{
		...
		$data['members'] = $this->member->get_data();



	    $this->load->view('admin/members/result', $data);
	...
	public function store() {
...

		{
			...
			$data = $this->member->get_status_data();
			...
			{		
...

		        {
		            ...
		            $data = array(
		            	...
		            	'fonts' => $this->member->fetch_fontData(),
		            ...
		            $this->load->view('admin/members/upload_result',$data);
...

    {
...

    	{
	    	...
	    	$this->member->store_template_data(implode(',', $fontName),implode(',', $fontType),implode(',', $fontColor),implode(',', $fontSize),implode(',', $fontCase),implode(',', $x),implode(',', $y),$file_name,$eventName);

	    	$data = $this->member->user_data();
...

		    {
		    	...
		    	//$coldata = [$value['name'],$value['course'],$value['email']];
                 $coldata = [$value['name'],$value['certificate_issue_date'],$value['certification_id']];
...

		    	{
		    		...
		    		$this->member->update_status($value['id'],$imgpath);
    ...
    {	
...

	    {
	    	...
	    	$fontFileName = $this->member->font_data($fontName[$i],$fontType[$i]);
		...
		$target_dir= str_replace(' ','_',"./assets/certificate/".$eventName."/");
...

	{
		...
		$themeData = $this->member->fetch_theme_data($this->input->get('themeid'));
